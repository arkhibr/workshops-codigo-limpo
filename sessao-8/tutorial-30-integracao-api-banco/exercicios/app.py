"""API de Pedidos com persistência sqlite — SUT do exercício-âncora (T30).

Cópia local de `exemplos/app.py` — o repositório é autocontido e nenhum
arquivo importa de outro diretório, então o SUT é replicado aqui, com a rota
adicional POST /pedidos/{id}/pagar usada pelo exercício.
...
def criar_app(conn: sqlite3.Connection) -> FastAPI:
    ...
    def pagar_pedido(pedido_id: int) -> dict:
        ...
        conn.commit()
        return {"id": linha["id"], "cliente": linha["cliente"],
                "total": linha["total"], "status": "pago"}
